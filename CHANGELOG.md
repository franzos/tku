...
### Fixed
- Live creds mirrored back into the active account's vault on every run — no more stale tokens after refresh or `claude /login`
- `tku account list` / `current` show live `subscriptionType` and `rateLimitTier` instead of values frozen at `account add`
- Previous vault contents kept at `<name>.previous.credentials.json` for one-step rollback
- `~/.claude.json` written `0o600` instead of `0o644`
